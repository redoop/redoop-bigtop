...
%define oozie_name oozie
%define usr_bin /usr/bin
%define lib_oozie %{crh_dir}/oozie
%define man_dir /usr/share/man
%define conf_oozie %{_sysconfdir}/%{oozie_name}/conf
%define conf_oozie_dist %{conf_oozie}.dist
%define tomcat_conf_oozie %{_sysconfdir}/%{oozie_name}/tomcat-conf
%define data_oozie /var/lib/oozie
%define lib_hadoop %{crh_dir}/hadoop
...
Name: oozie%{crh_version_as_name}
...
Buildroot: %{_topdir}/INSTALL/%{oozie_name}-%{version}
License: ASL 2.0
Source0: %{oozie_name}-%{oozie_base_version}.tar.gz
...
Requires: oozie%{crh_version_as_name}-client = %{version}, hadoop%{crh_version_as_name}-client, bigtop-tomcat
BuildArch: noarch
...
sed -i -e "s,{CRH_DIR},%{crh_dir}," $RPM_SOURCE_DIR/*
    env CRH_DIR=%{crh_dir} sh %{SOURCE2} --extra-dir=$RPM_SOURCE_DIR --build-dir=$PWD --server-dir=$RPM_BUILD_ROOT --client-dir=$RPM_BUILD_ROOT --docs-dir=$RPM_BUILD_ROOT%{doc_oozie} --initd-dir=$RPM_BUILD_ROOT%{initd_dir} --conf-dir=$RPM_BUILD_ROOT%{conf_oozie_dist}
...
if [ !  -e "/etc/oozie/conf" ]; then
     rm -f /etc/oozie/conf
     mkdir -p /etc/oozie/conf
     cp -rp /etc/oozie/conf.dist/* /etc/oozie/conf 
fi



if [ !  -e "%{lib_oozie}/oozie-server/conf" ]; then
     rm -f %{lib_oozie}/oozie-server/conf
     ln -s /etc/oozie/tomcat-conf.https/conf/ %{lib_oozie}/oozie-server/conf
fi



%{alternatives_cmd} --install %{conf_oozie} %{oozie_name}-conf %{conf_oozie_dist} 30
%{alternatives_cmd} --install %{tomcat_conf_oozie} %{oozie_name}-tomcat-conf %{tomcat_conf_oozie}.http 30
%{alternatives_cmd} --install %{tomcat_conf_oozie} %{oozie_name}-tomcat-conf %{tomcat_conf_oozie}.https 20
...
if [ "$1" = 0 ]; then
  ...
  %{alternatives_cmd} --remove %{oozie_name}-tomcat-conf %{tomcat_conf_oozie}.http || :
  %{alternatives_cmd} --remove %{oozie_name}-tomcat-conf %{tomcat_conf_oozie}.https || :
  %{alternatives_cmd} --remove %{oozie_name}-conf %{conf_oozie_dist} || :
fi
