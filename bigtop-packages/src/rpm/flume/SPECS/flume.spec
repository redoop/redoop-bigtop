...
Name: flume%{crh_version_as_name}
...
Buildroot: %{_topdir}/INSTALL/%{flume_name}-%{version}
BuildArch: noarch
License: ASL 2.0
Source0: %{flume_folder}.tar.gz
Source1: do-component-build
Source2: install_%{flume_name}.sh
Source3: %{flume_name}-agent.init
Source4: flume-agent.default
#BIGTOP_PATCH_FILES
Requires: /usr/sbin/useradd
Requires: coreutils
Requires: hadoop%{crh_version_as_name}, hadoop%{crh_version_as_name}-hdfs 
...
Group: Development/Libraries
BuildArch: noarch
Requires: %{flume_name} = %{version}-%{release}
Requires: /sbin/service
Requires(pre): %{flume_name} = %{version}-%{release}
...
# Install init script
init_file=$RPM_BUILD_ROOT/%{initd_dir}/%{flume_name}-agent
...
%__cp %{SOURCE4} $RPM_BUILD_ROOT/etc/default/%{flume_name}-agent
...
if [ !  -e "/etc/flume/conf" ]; then
     rm -f /etc/flume/conf
     mkdir -p /etc/flume/conf
     cp -rp /etc/flume/conf.empty/* /etc/flume/conf 
fi

%{alternatives_cmd} --install %{etc_flume} %{flume_name}-conf %{etc_flume}.empty 30

%preun
if [ "$1" = 0 ]; then
        %{alternatives_cmd} --remove %{flume_name}-conf %{etc_flume}.empty || :
fi

%post agent
chkconfig --add %{flume_name}-agent

%preun agent
if [ $1 = 0 ] ; then
        service %{flume_name}-agent stop > /dev/null 2>&1
        chkconfig --del %{flume_name}-agent
fi
%postun agent
if [ $1 -ge 1 ]; then
        service %{flume_name}-agent condrestart >/dev/null 2>&1
fi
...
%files agent
%attr(0755,root,root)/%{initd_dir}/%{flume_name}-agent
%attr(0644,root,root) %config(noreplace) /etc/default/%{flume_name}-agent
