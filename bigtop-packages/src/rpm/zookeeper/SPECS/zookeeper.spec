...
%define etc_zookeeper /etc/%{zookeeper_name}
%define bin_zookeeper %{_bindir}
%define lib_zookeeper %{crh_dir}/%{zookeeper_name}
%define log_zookeeper /var/log/%{zookeeper_name}
%define run_zookeeper /var/run/%{zookeeper_name}
%define vlb_zookeeper /var/lib/%{zookeeper_name}
...
%define doc_zookeeper %{_docdir}/%{zookeeper_name}
...
%define doc_zookeeper %{_docdir}/%{zookeeper_name}-%{zookeeper_version}
...
Name: zookeeper%{crh_version_as_name}
Version: %{zookeeper_version}
Release: %{zookeeper_release}
Summary: A high-performance coordination service for distributed applications.
URL: http://zookeeper.apache.org/
Group: Development/Libraries
Buildroot: %{_topdir}/INSTALL/%{zookeeper_name}-%{version}
License: ASL 2.0
Source0: %{zookeeper_name}-%{zookeeper_base_version}.tar.gz
...
%prep
%setup -n %{zookeeper_name}-%{zookeeper_base_version}
...
env CRH_DIR=%{crh_dir} bash %{SOURCE2} \
          --build-dir=build/%{zookeeper_name}-%{zookeeper_base_version} \
...
sed -i -e "s,{CRH_DIR},%{crh_dir}," $RPM_SOURCE_DIR/zookeeper-rest.svc
...
if [ !  -e "/etc/zookeeper/conf" ]; then
     ...
     cp -rp /etc/zookeeper/conf.dist/* /etc/zookeeper/conf 
fi 

/usr/bin/%{distroselect} --rpm-mode set zookeeper-client %{crh_version_with_bn}
/usr/bin/%{distroselect} --rpm-mode set zookeeper-server %{crh_version_with_bn}

%{alternatives_cmd} --install %{etc_zookeeper}/conf %{zookeeper_name}-conf %{etc_zookeeper}/conf.dist 30
%__install -d -o zookeeper -g zookeeper -m 0755 %{vlb_zookeeper}

%preun
if [ "$1" = 0 ]; then
        %{alternatives_cmd} --remove %{zookeeper_name}-conf %{etc_zookeeper}/conf.dist || :
fi
