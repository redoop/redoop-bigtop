...
Name: sqoop%{crh_version_as_name}
...
Buildroot: %{_topdir}/INSTALL/%{sqoop_name}-%{version}
License: ASL 2.0
Source0: %{sqoop_name}-%{sqoop_base_version}.tar.gz
Source1: do-component-build
Source2: install_%{sqoop_name}.sh
...
%prep
%setup -n %{sqoop_name}-%{sqoop_base_version}
...
if [ !  -e "/etc/sqoop/conf" ]; then
     rm -f /etc/sqoop/conf
     mkdir -p /etc/sqoop/conf
...
%preun
if [ "$1" = 0 ]; then
  %{alternatives_cmd} --remove %{sqoop_name}-conf %{conf_sqoop_dist} || :
fi
