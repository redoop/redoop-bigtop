...
Name: kafka%{crh_version_as_name}
...
env CRH_DIR=%{crh_dir} bash $RPM_SOURCE_DIR/install_kafka.sh \
          --build-dir=`pwd`         \
...
if [ !  -e "/etc/kafka/conf" ]; then
     rm -f /etc/kafka/conf
     mkdir -p /etc/kafka/conf
     cp -rp /etc/kafka/conf.dist/* /etc/kafka/conf 
fi
