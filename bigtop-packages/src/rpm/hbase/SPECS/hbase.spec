...
%define hbase_home %{crh_dir}/%{hbase_name}
...
%define doc_hbase %{_docdir}/%{hbase_name}
...
%define doc_hbase %{_docdir}/%{hbase_name}-%{hbase_version}
...
Name: hbase%{crh_version_as_name}
...
Buildroot: %{_topdir}/INSTALL/%{hbase_name}-%{version}
License: ASL 2.0
...
env CRH_DIR=%{crh_dir} CRH_VERSION=%{crh_version_with_bn} bash %{SOURCE2} \
	--build-dir=build \
        --doc-dir=%{doc_hbase} \
...
%__install -m 0644 %{SOURCE5} $RPM_BUILD_ROOT/etc/default/%{hbase_name}

%__install -d -m 0755 $RPM_BUILD_ROOT/etc/security/limits.d
%__install -m 0644 %{SOURCE6} $RPM_BUILD_ROOT/etc/security/limits.d/%{hbase_name}.nofiles.conf

%__install -d  -m 0755  %{buildroot}/%{_localstatedir}/log/%{hbase_name}
ln -s %{_localstatedir}/log/%{hbase_name} %{buildroot}/%{logs_hbase}

%__install -d  -m 0755  %{buildroot}/%{_localstatedir}/run/%{hbase_name}
ln -s %{_localstatedir}/run/%{name} %{buildroot}/%{pids_hbase}

%__install -d  -m 0755  %{buildroot}/%{_localstatedir}/lib/%{hbase_name}
...
%preun
if [ "$1" = 0 ]; then
        %{alternatives_cmd} --remove %{hbase_name}-conf %{etc_hbase_conf_dist} || :
fi
