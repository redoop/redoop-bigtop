...
%define lib_pig %{crh_dir}/%{pig_name}
...
Buildroot: %{_topdir}/INSTALL/%{pig_name}-%{version}
BuildArch: noarch
...
%prep
%setup -n %{pig_name}-%{pig_base_version}-src
...
env CRH_DIR=%{crh_dir} PIG_VERSION=%{pig_version} PIG_JAR_VERSION=%{pig_jar_version} CRH_VERSION=%{crh_version_with_bn} sh -x %{SOURCE2} \
          --build-dir=build \
          --doc-dir=%{doc_pig} \
