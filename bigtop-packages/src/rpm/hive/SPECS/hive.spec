...
%define usr_lib_hive %{crh_dir}/%{hive_name}
%define usr_lib_hcatalog %{crh_dir}/hive-hcatalog
%define var_lib_hive /var/lib/%{hive_name}
%define var_lib_hcatalog /var/lib/%{hive_name}-hcatalog
%define var_log_hcatalog /var/log/%{hive_name}-hcatalog
...
%define doc_hive %{_docdir}/%{hive_name}-%{hive_version}
...
%define doc_hive %{_docdir}/%{hive_name}
...
Buildroot: %{_topdir}/INSTALL/%{hive_name}-%{version}
BuildArch: noarch
...
Source18: bigtop.bom
#BIGTOP_PATCH_FILES
Requires: hadoop%{crh_version_as_name}-client, bigtop-utils >= 0.7, zookeeper%{crh_version_as_name}, hive%{crh_version_as_name}-jdbc = %{version}-%{release}, spark%{crh_version_as_name}
...
Group: Development/Libraries
Requires: hadoop%{crh_version_as_name}, %{name}, bigtop-utils >= 0.7
...
%__install -m 0644 $RPM_SOURCE_DIR/hive-metastore.default $RPM_BUILD_ROOT/etc/default/%{hive_name}-metastore
%__install -m 0644 $RPM_SOURCE_DIR/hive-server2.default $RPM_BUILD_ROOT/etc/default/%{hive_name}-server2
%__install -m 0644 $RPM_SOURCE_DIR/hive-hcatalog-server.default $RPM_BUILD_ROOT/etc/default/%{hive_name}-hcatalog-server
%__install -m 0644 $RPM_SOURCE_DIR/hive-webhcat-server.default $RPM_BUILD_ROOT/etc/default/%{hive_name}-webhcat-server

%__install -d -m 0755 $RPM_BUILD_ROOT/%{_localstatedir}/log/%{hive_name}
%__install -d -m 0755 $RPM_BUILD_ROOT/%{_localstatedir}/run/%{hive_name}
...
%{usr_lib_hive}
%{usr_lib_hive}/hive.tar.gz
%{usr_bin}/hive
%{usr_bin}/beeline
%{usr_bin}/hiveserver2
%attr(0755,hive,hive) %dir %{var_lib_hive}
%attr(0755,hive,hive) %dir %{_localstatedir}/log/%{hive_name}
%attr(0755,hive,hive) %dir %{_localstatedir}/run/%{hive_name}
%doc %{doc_hive}
%{man_dir}/man1/hive.1.*
...
%config(noreplace) /etc/default/%{hive_name}-%1 \
%post %1 \
