...
usage: $0 <options>
  ...
  Optional options:
     ...
     --lib-dir=DIR               path to install zookeeper home [${CRH_DIR}/zookeeper]
...
LIB_DIR=${LIB_DIR:-${CRH_DIR}/zookeeper}
...
export ZOOKEEPER_HOME=\${ZOOKEEPER_CONF:-${CRH_DIR}/zookeeper}
...
env CLASSPATH=\$CLASSPATH ${CRH_DIR}/zookeeper/bin/zkCli.sh "\$@"
EOF
chmod 755 $wrapper
...
env CLASSPATH=\$CLASSPATH ${CRH_DIR}/zookeeper/bin/${upstream_script} "\$@"
EOF
  chmod 755 $wrapper
done
...
${CRH_DIR}/zookeeper-native/`basename ${binary}` \$@
