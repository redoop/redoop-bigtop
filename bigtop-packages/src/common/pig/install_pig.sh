...
usage: $0 <options>
  ...
  Optional options:
     ...
     --lib-dir=DIR               path to install pig home [${CRH_DIR}/pig]
...
LIB_DIR=${LIB_DIR:-${CRH_DIR}/pig}
INSTALLED_LIB_DIR=${INSTALLED_LIB_DIR:-${CRH_DIR}/pig}
...
export PIG_HOME=\${PIG_HOME:-${CRH_DIR}/pig}
export HADOOP_PREFIX=\${HADOOP_PREFIX:-${CRH_DIR}/hadoop}
export HADOOP_CONF_DIR=\${HADOOP_CONF_DIR:-/etc/hadoop/conf}
export HBASE_HOME=\${HBASE_HOME:-${CRH_DIR}/hbase}
export HBASE_CONF_DIR=\${HBASE_HOME}/conf
export ZOOKEEPER_HOME=\${ZOOKEEPER_HOME:-${CRH_DIR}/zookeeper}
export HIVE_HOME=\${HIVE_HOME:-${CRH_DIR}/hive}
export HCAT_HOME=\${HCAT_HOME:-${CRH_DIR}/hive-hcatalog}
...
sed -i -e "s|../build/ivy/lib/Pig|${CRH_DIR}/pig/lib/|" $PREFIX/$EXAMPLES_DIR/build.xml
sed -i -e "s|<fileset dir=\"..\">|<fileset dir=\"${CRH_DIR}/pig/\">|" $PREFIX/$EXAMPLES_DIR/build.xml
sed -i -e "s|<path id=\"tutorial.classpath\">|<path id=\"tutorial.classpath\"> <fileset dir=\"${CRH_DIR}/hadoop\"> <include name=\"*.jar\"/> </fileset>|" $PREFIX/$EXAMPLES_DIR/build.xml

# Pig log directory
install -d -m 1777 $PREFIX/var/log/pig

#Copying pig.tar.gz for use by webhcat 
ABS_PIG_DIR=$(cd $PREFIX/${LIB_DIR} && pwd) 
(cd $BUILD_DIR/tar/pig* && tar -cz --exclude-vcs --transform 's,^\.,pig,' -f ${ABS_PIG_DIR}/pig.tar.gz .)
