...
export HADOOP_MAPRED_HOME=\${HADOOP_MAPRED_HOME:-$CRH_DIR/hadoop-mapreduce}
export HADOOP_YARN_HOME=\${HADOOP_YARN_HOME:-$CRH_DIR/hadoop-yarn}
export HADOOP_LIBEXEC_DIR=\${HADOOP_HOME}/libexec
export CRH_VERSION=\${CRH_VERSION:-${CRH_VERSION}}
export HADOOP_OPTS="\${HADOOP_OPTS} -Dcrh.version=\${CRH_VERSION}"
export YARN_OPTS="\${YARN_OPTS} -Dcrh.version=\${CRH_VERSION}"
...
#hadoop build gneratess ${BUILD_DIR}
# Rm src/* and tar that ${BUILD_DIR} as mapreduce.tar.gz
# Rename the parent directory to hadoop instead of hadoop-<version>
pushd ${BUILD_DIR}
rm -rf src/*
tar -cz  --transform 's,^\.,hadoop,' -f ${HADOOP_DIR}/mapreduce.tar.gz .
popd
