...
usage: $0 <options>
  ...
  Optional options:
     ...
     --lib-dir=DIR               path to install sqoop home [$CRH_DIR/sqoop]
...
LIB_DIR=${LIB_DIR:-$CRH_DIR/sqoop}
BIN_DIR=${BIN_DIR:-$CRH_DIR/sqoop/bin}
...
#Creating sqoop for use by webhcat etc., 
ABS_SQOOP_DIR=$(cd ${PREFIX}/${LIB_DIR} && pwd) 
(cd $BUILD_DIR && tar -cz --exclude='./debian' --exclude='./src' --exclude='./docs'  --exclude-vcs --transform 's,^\.,sqoop,' -f ${ABS_SQOOP_DIR}/sqoop.tar.gz .)
