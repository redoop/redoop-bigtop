...
def spark_service(name, upgrade_type=None, action=None):
  ...
  if action == 'start':
    ...
    if name == 'jobhistoryserver' and effective_version and check_stack_feature(StackFeature.SPARK_16PLUS, effective_version):
      # create & copy spark-crh-yarn-archive.tar.gz to hdfs
    ...
    # Spark 1.3.1.2.3, and higher, which was included in CRH 5.0, does not have a dependency on Tez, so it does not
