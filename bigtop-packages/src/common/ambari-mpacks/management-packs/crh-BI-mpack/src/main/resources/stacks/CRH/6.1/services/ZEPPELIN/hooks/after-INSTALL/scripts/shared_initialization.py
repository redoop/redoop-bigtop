...
def setup_stack_symlinks():
  """
  This should always be called after a component has been installed to ensure that all CRH pointers are correct. 
  The stack upgrade logic does not interact with this since it's done via a custom command and will not trigger this hook.
  :return:
  """
  import params
  if params.stack_version_formatted != "" and compare_versions(params.stack_version_formatted, '5.0') >= 0:
    ...
    if not params.upgrade_suspended:
      if params.host_sys_prepped:
        Logger.warning("Skipping running stack-selector-tool for stack {0} as its a sys_prepped host. This may cause symlink pointers not to be created for CRH componets installed later on top of an already sys_prepped host.".format(version))
        return
...
def link_configs(struct_out_file):
  """
  Links configs, only on a fresh install of CRH-5.0 and higher
  """
  import params

  if not Script.is_stack_greater_or_equal("5.0"):
    Logger.info("Can only link configs for CRH-5.0 and higher.")
    return
