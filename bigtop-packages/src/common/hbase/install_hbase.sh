...
usage: $0 <options>
  ...
  Optional options:
     ...
     --lib-dir=DIR               path to install hbase home [$CRH_DIR/hbase]
...
LIB_DIR=${LIB_DIR:-$CRH_DIR/hbase}

BIN_DIR=${BIN_DIR:-$CRH_DIR/hbase/bin}
...
popd
mv $PREFIX/$BIN_DIR/hbase $PREFIX/$BIN_DIR/hbase.distro
...
export HBASE_OPTS="-Dcrh.version=\${CRH_VERSION} \${HBASE_OPTS}"

exec $CRH_DIR/hbase/bin/hbase.distro "\$@"
EOF
chmod 755 $wrapper
cp $PREFIX/usr/bin/hbase $PREFIX/${BIN_DIR}/
install -d -m 0755 $PREFIX/usr/bin
install -d -m 0755 $PREFIX/usr/sw
